...
class GroupController extends Controller
{
    ...
    public function create()
    {
        ...
        $contacts = $user->familyRelations()
            ->with('relatedUser')
            ->get()
            ->map(function ($relation) {
                return [
                    'id' => $relation->relatedUser->id,
                    'name' => $relation->relatedUser->name,
                    'avatar' => $relation->relatedUser->profile?->avatar_url,
                    'relation' => $relation->relationshipType->display_name_fr ?? $relation->relationshipType->name,
                ];
            })
            ->merge(
                $user->reverseRelations()
                    ->with('user')
                    ->get()
                    ->map(function ($relation) {
                        return [
                            ...
                            'avatar' => $relation->user->profile?->avatar_url,
                            'relation' => $relation->relationshipType->display_name_fr ?? $relation->relationshipType->name,
                        ];
                    })
            )
